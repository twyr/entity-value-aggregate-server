...
export class Masterdata extends BaseMiddleware {
	...
	async #getConnectionStatuses(id) {
		const masterdata = await this.#getMasterdata(
	...
	async #getGenders(id) {
		const masterdata = await this.#getMasterdata('gender_master', 'id', id);
		return {
			status: 200,
			body: masterdata
		};
	...
	async #getLocales(id) {
		let masterdata = await this.#getMasterdata('locale_master', 'id', id);

		masterdata = masterdata?.filter?.((locale) => {
			return locale?.is_enabled;
		});
	...
	async #getMasterdata(tableName, field, id) {
		const dbRepository =
			await this.domainInterface?.iocContainer?.resolve?.('SQLDatabase');

		let masterdata = await this?._executeWithBackOff?.(async () => {
			if (!id) {
				return dbRepository?.raw?.(`SELECT * FROM ${tableName}`);
			} else {
...
export default class MiddlewareFactory extends EVASBaseFactory {
	...
	static async createInstances(domainInterface) {
		if (!MiddlewareFactory.#masterdataInstance) {
			...
			MiddlewareFactory.#masterdataInstance = masterdataInstance;
		}

		return MiddlewareFactory.#masterdataInstance;
	}
	...
	static async destroyInstances() {
		await MiddlewareFactory.#masterdataInstance?.unload?.();
		MiddlewareFactory.#masterdataInstance = undefined;
